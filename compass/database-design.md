...
| 과제 | 설계 방안 | 설계 구조 | 핵심 질문 |
|------|----------|----------|----------|
| 지표의 유연한 관리 | 지표를 코드가 아닌 데이터로 관리 | 카테고리-지표-값 3단계 구조 | 무엇을 비교할 것인가? |
| 다양한 관점의 기업 비교 | 공식 분류와 분석 목적별 분류 분리 | 공식 분류 + 테마 분류 이원화 | 누구를 비교할 것인가? |
| 그룹사 연계 분석 | 관계 정보를 별도 구조로 관리하고 이력 보존 | 관계 테이블 + 유효 기간 | 누구를 비교할 것인가? |
| 순위 산출과 변동 추적 | 리그테이블 정의와 계산 결과 분리 | 정의-스냅샷-항목 3단계 구조 | 어떻게 비교할 것인가? |
