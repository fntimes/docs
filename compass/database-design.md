...
Compass의 핵심 콘텐츠는 의미있는 비교가 가능한 기업들을 그룹화하여 분석하고 다양한 각도에서 순위를 매기는 것. 이를 위해 주요 과제를 설정하고 각각에 맞는 설계 방안을 적용함.
...
지표를 코드에 고정하면 변경 시마다 배포가 필요하므로, 데이터로 관리하여 운영 중에도 유연하게 대응. 카테고리로 그룹화하여 개별 지표 비교와 카테고리 단위 비교 모두 지원.
