...
Compass의 핵심 콘텐츠는 의미있는 비교가 가능한 기업들을 그룹화하여 분석하고 다양한 각도에서 순위를 매기는 것. 이를 위해 네 가지 과제를 설정하고 각각에 맞는 설계 방안을 적용함.
...
- KB금융지주 선택 시 자회사 지표를 함께 비교 가능
- 4대 금융지주의 은행 자회사끼리 비교 가능
- 특정 시점 기준 그룹 구조 조회 가능
...
- 시점별 순위 변동 추적 가능 (예: "지난 분기 3위 → 이번 분기 1위")
